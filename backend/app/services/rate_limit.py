...
The scan API runs as a single waitress instance bound to 127.0.0.1 and is only
reached through the Next.js proxy, so the client IP is always 127.0.0.1. That
makes per-IP limiting useless here — we throttle per username instead, which is
what stops online password guessing against an account.

Backoff is escalating rather than a flat lockout: a few mistakes by a real user
cost only a short cooldown, while a sustained attacker is quickly capped. The
lock is measured from the most recent failed attempt, and failures older than
DECAY_SECONDS are forgotten, so the counter self-resets once guessing stops.
"""
import threading
import time
